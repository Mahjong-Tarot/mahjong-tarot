...
## First-run protocol (runs once — when docs/product/overview.md does not exist)

Ask the stakeholder these 5 questions before writing anything:

1. **Business purpose** — What does this product do and who is it for?
2. **Brand and tone** — How should the brand feel? (words, colours, references)
3. **Top 3 outcomes** — What does success look like in 90 days?
4. **Known constraints** — Budget, timeline, things we must not change?
5. **First epic** — What is the single most important thing to build first?

After receiving answers, scaffold the following files:
- `docs/product/overview.md` — business purpose, target user, brand brief
- `docs/product/epics.md` — epics derived from outcomes, each with acceptance criteria
- `docs/product/roadmap.md` — phased timeline: now / next / later
- `docs/project-status.html` — single-file project dashboard (open in browser)
  - Sections: Active Sprint, Epics Status, Recent Commits, Blockers
  - Status badges per epic: 🔴 Blocked / 🟡 In Progress / 🟢 Done / ⚪ Not Started

## Epic and user story format
Every epic must contain:
- **Hypothesis**: "We believe that [doing X] will result in [outcome] for [user]"
- **Acceptance criteria**: Specific, measurable conditions — not feelings or intentions
- **Definition of done**: What a reviewer can check to confirm it's complete
- **Priority signal**: Why this before anything else (data, constraint, or dependency)

Every user story must answer four things:
- Who: the specific user type, not "a user"
- What: the action they need to take
- Why: the outcome they expect — the job to be done
- Success measure: how we know the story delivered value
