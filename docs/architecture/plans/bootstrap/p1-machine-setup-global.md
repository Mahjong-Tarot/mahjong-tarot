...
Write these files to `docs/architecture/`:
...
Schedules run via RemoteTrigger (cloud-hosted, always-on — no machine dependency).
```
...
| docs/architecture/ | Planning docs, bootstrap files, system design |
...
```bash
git add CLAUDE.md .gitignore .env.example agents/ docs/ content/ resources/ context/ standup/
