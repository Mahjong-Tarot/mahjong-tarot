...
Here's what I'm setting up:
 ...
 8. Write Supabase credentials to .env (skipped if not set up yet)
 9. Supabase MCP connection (skipped if not set up yet)
10. Install skills (create-local-task, daily-checkin, skill-creator)
11. Initial commit
12. GitHub repo creation + Vercel import

I'll ask you 7 questions, then run everything automatically.
```
...
1. Your GitHub repo name (e.g. "acme-marketing" — no spaces, becomes your folder name too)
2. Your name/handle for git (e.g. "jsmith")
3. Your email for git (e.g. jane@example.com)
4. Your Supabase Project URL from P0 Step 4 — or type "skip" if not set up yet
5. Your Supabase anon key from P0 Step 4 — or "skip"
6. Your Supabase service role key from P0 Step 4 — or "skip"
7. Your Supabase personal access token from P0 Step 4 (starts with "sbp_") — or "skip"
8. Did you set up Lark in P0 Step 6? (yes / no)
```
...
```bash
gh auth status 2>/dev/null || gh auth login --web --git-protocol https
```
...
## STEP 5 — Git identity + credential helper

```bash
git config --global user.name  "{handle from answer 2}"
git config --global user.email "{email from answer 3}"
git config --global credential.helper "$(which gh) auth git-credential"
```

> **Why the credential helper?** macOS prompts for your keychain password every time git
> accesses GitHub credentials. Using `gh auth git-credential` bypasses the keychain entirely
> and uses gh's stored token — no prompts.
...
## STEP 11 — .env template + credentials
...
**If answers 4–6 were provided** (not "skip"), write `.env` with real values:
...
**If answers 4–6 were "skip"**, write `.env` with blank placeholders and print:

```
⚠️  Supabase credentials not added — complete P0 Step 4 and run the Supabase setup step separately.
```
...
**Skip this entire step if answer 7 was "skip".**
...
python3 - <<PYEOF
import json, os
token = "{supabase personal access token from answer 7}"
path = os.path.expanduser("~/.claude/settings.local.json")
settings = {}
if os.path.exists(path):
    with open(path) as f:
...
## STEP 13 — vercel.json (NEW — prevents Vercel root directory issue)

Write `vercel.json` to the repo root. This tells Vercel where the Next.js app lives
so the user never has to find the Root Directory setting during import:
...
```bash
git add CLAUDE.md .gitignore .env.example vercel.json .claude/ agents/ architecture/ content/ resources/ context/ standup/
git commit -m "bootstrap: P1 complete — structure, config, skills"
```

---

## STEP 16 — GitHub repo creation + Vercel import
...
2. Under "Import Git Repository", find {repo-name} and click Import
3. On the Configure Project screen:
   - Framework Preset will show "Next.js" — this is correct, leave it
   - Root Directory is already set via vercel.json — do NOT change it
   - Skip environment variables for now if Supabase isn't set up yet
4. Click Deploy

The first deploy takes ~1 minute. Once it shows "Congratulations!", your site is live.
The URL will be: https://{repo-name}.vercel.app (or similar — Vercel will show it)
